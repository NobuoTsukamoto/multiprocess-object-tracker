...
- **オープン失敗の GUI 通知（確定）**: プロセスの自然死とエラー終了を区別するため、専用 `Event` か ステータス Queue で GUI に通知（R-CAM-14、機構は要確認）。GUI は既にプロセス `is_alive` を監視する（`src/gui_controller.py:478`）が、死活だけではエラー要因を区別できないため明示通知を足す。
