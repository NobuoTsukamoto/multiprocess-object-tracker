...
[`tests/test_camera_controller.py`](../../../tests/test_camera_controller.py) は純関数寄りの `_fit_to_pool`（resize / チャンネル不一致）をカバーする。`run()` の経路（open/grab/write/finally）は実カメラ依存のため未カバー（`cv2.VideoCapture` のモック化が要る）。
...
- [ ] `run()` 経路を `cv2.VideoCapture` モックで補完。
  - [ ] open 失敗時に error ログ＋`_report_error`＋プール close＋早期 return（R-CAM-04/14）。
  - [ ] grab 失敗（ret=False）で warning＋sleep＋継続（R-CAM-07）。
  - [ ] `_fit_to_pool` が `None` を返すフレームで error ログ＋`continue`＋`frame_id` 非加算（R-CAM-15）。
