...
### 文書化 / 整合
- [ ] **README 設定表を実態に同期**（`README.md:89-113`）: 欠落している `tracking.frame_read_policy` / `tracking.max_frame_skip` / `gui.frame_buffer_seconds` を追加。
...
- [ ] `src/config_manager.py:24` の `fp16` を削除（FP16 はモデル側で対応）。
- [ ] `config/default.yaml:10` の `fp16` 行を削除。
- [ ] `README.md:97` の `fp16` 行を削除。
- [ ] `src/config_manager.py:35` の `max_track_num` を削除。
- [ ] `config/default.yaml:27` の `max_track_num` 行を削除。
- [ ] `README.md:104` の `max_track_num` 行を削除。

### 実装（✅確定）
- [ ] **NMS 関連の設定キー化**: `DetectionConfig` に `detection_threshold`（既定 0.1）・`nms_iou_threshold`（既定 0.45）を追加し、`object_tracking_controller.py:189-190` のハードコードを差し替え。`default.yaml`・README 設定表も同期。（命名確定）
- [ ] **空ファイル時の専用例外**: `_load_config`（`src/config_manager.py:71-74`）で `config_dict is None` を検出し、専用例外 `EmptyConfigError`（仮称）をメッセージ付きで送出。`main.py` のハンドリングも併せて確認。テスト R-CM-09 を改修後の挙動へ更新。
