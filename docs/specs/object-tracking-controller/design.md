...
`object-tracking-controller` は推論+追跡のワーカープロセスである。`multiprocessing.Process` を継承し、`run()` で ONNX セッション（YOLOX）と `supervision.ByteTrack` を構築し、追跡用共有メモリプールからフレームを読み出して「前処理 → 推論 → YOLOX 後処理 → 検出フィルタ → NMS → 追跡」を回す。結果は `TrackingResult` にまとめ、`track_queue`（`multiprocessing.Queue`）で GUI に送る。出典 `src/object_tracking_controller.py:25-274`。
...
| `run` | セッション/追跡器構築→ループ→送出→後始末 | `src/object_tracking_controller.py:124-274` |
...
出典: `src/object_tracking_controller.py:153-271`。なお ONNX ロード失敗時はループに入る前に `_report_error`→`return`（`:133-136`）。
...
- **`frame_id` 突合**: `TrackingResult.frame_id == FrameRef.frame_id`。出典 `:231`。
- **レイテンシ恒等式**: `total == queue + process`（同一 `start_time`/`end_time`/`timestamp`）。出典 `:163-165,226-228`。
- **新しさ優先**: `track_queue` は最大 `max_queue_length`。Full 時は最古を捨て最新を入れる。出典 `:240-252`、`src/gui_controller.py:77-79`。
...
- **空検出 / tracker_id None**: `track_infos` 空のまま `TrackingResult` を送出。出典 `:217-224`。
- **track_queue Full**: 最古を捨てて再 put、なお Full なら warning。出典 `:240-252`。
...
- **検出閾値の設定キー化（実装済み）**: 生検出フィルタ `0.1`・NMS IoU `0.45` を [`config-manager`](../config-manager/) の `detection.detection_threshold`/`nms_iou_threshold` へキー化し、消費を `self.det_config.*` へ差し替え済み。出典 `:205,208`。
...
- `src/object_tracking_controller.py:25-274` — 本体
...
- `src/config_manager.py:22-41` — `DetectionConfig`/`TrackingConfig`
