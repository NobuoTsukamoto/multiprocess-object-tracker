...
- ✅ **座標系は `xyxy`**（左上 x1,y1・右下 x2,y2、ピクセル単位）: `TrackInfo.box`/`DetectionResult.boxes` は削除済みのため、xyxy セマンティクスは `detections`（`sv.Detections.xyxy`）にのみ残る。出典 `src/object_tracking_controller.py:190-195`。
