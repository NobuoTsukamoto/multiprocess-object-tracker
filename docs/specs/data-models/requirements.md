...
- ✅ **`TrackingResult.detections` は `Any` 型を維持する（方針確定）**: 実体は `sv.Detections`（追跡後の `tracked_detections`）で、Queue で pickle 転送され、GUI が `len()`/`.confidence`/`.class_id`/`.tracker_id` と annotator 再投入で消費する（出典 `src/gui_controller.py:607,642-655`）。
  ...
  - **方針成立の前提条件（整備済み）**: 「テストが通れば OK」を機能させる2本のガードレール — (1) `TrackingResult`（実 `sv.Detections` を含む）の pickle 往復テスト（`DetectionsPickleRoundTripTest`）、(2) GUI 描画経路を実 `sv.Detections` で通すスモークテスト（`RenderImageSmokeTest`）— を**整備済み**。supervision 更新でこの API が壊れるとアプリではなく CI が先に落ちる。
