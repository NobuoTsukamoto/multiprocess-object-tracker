...
| 要素 | 役割 | 出典 |
|:--|:--|:--|
| `SharedFramePool` | owner。共有メモリと free_queue を生成（data_queue は呼び出し側から受け取る）し、active 状態と reset/cleanup を管理 | `shared_frame_pool.py:67-150` |
...
- **3 つの資源**: ① 共有メモリ N ブロック（`self.shms`）、② `free_queue`（空きスロット番号）、③ `data_queue`（書き込み済み `FrameRef`）。①② は owner が生成、③ はコンストラクタ引数で呼び出し側から注入される（owner は生成しない）。出典 `:70-88`。
